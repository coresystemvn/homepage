...
| Khả năng | Mô tả |
|----------|-------|
| Theo dõi lịch sử Deployment | Hiển thị chi tiết danh sách thiết bị đã cài đặt: model máy, CPU, dung lượng RAM, cấu trúc ổ đĩa, USB boot sử dụng, cùng phiên bản OS build/edition/language/version. |
...
| Reset API key chủ động | Khi nghi ngờ API key đang bị rò rỉ và lạm dụng, MSP có thể reset API key chủ động. Toàn bộ các USB sử dụng API key cũ sẽ bị vô hiệu hóa |

## Quy trình đăng nhập

1. Truy cập vào địa chỉ **`https://msp.coresystem.vn`**.
2. Hệ thống xác thực Cloudflare Access sẽ hiển thị giao diện đăng nhập → Nhập địa chỉ **email** đã đăng ký → Nhận mã xác minh (OTP) gửi qua email.
3. Nhập mã OTP chính xác để đăng nhập thành công → Giao diện Dashboard sẽ hiển thị theo phân quyền tài khoản của bạn.

:::tip
Để đăng xuất khỏi hệ thống: Click nút **Log out** trên giao diện. Ngoài ra, phiên làm việc (session) sẽ tự động hết hạn (timeout) sau 30 phút không hoạt động.
:::

## Vai trò tài khoản và Phạm vi dữ liệu

:::note
**Web Dashboard chỉ áp dụng với gói dịch vụ MSP Advanced.** Về mặt kỹ thuật, mọi gói dịch vụ (tier) đều tự động gửi dữ liệu triển khai về hệ thống máy chủ (telemetry) — tuy nhiên, quyền hạn truy cập giao diện Dashboard được phân định cụ thể theo gói dịch vụ:
